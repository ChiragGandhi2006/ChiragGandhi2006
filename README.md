<div align="center">

![Chirag Gandhi](https://raw.githubusercontent.com/ChiragGandhi2006/ChiragGandhi2006/main/profile.jpg)

# Chirag Gandhi

### AI/ML Engineer | Machine Learning | Deep Learning | Data Science

**Building intelligent systems with Machine Learning, Deep Learning & Data.**

[GitHub](https://github.com/ChiragGandhi2006) • [LinkedIn](https://www.linkedin.com/in/chirag-gandhi-672a67372/) • [LeetCode](https://leetcode.com/u/CKhjBWYKSg/) • [Email](mailto:gandhichirag2006@gmail.com) • [Resume](https://github.com/ChiragGandhi2006/ChiragGandhi2006/blob/main/RESUME.md)

</div>

---

## About Me

I'm **Chirag Gandhi**, a B.Tech student specializing in **Artificial Intelligence & Data Science at Vishwakarma Institute of Technology, Pune**.

- I build **Machine Learning and AI applications** that solve practical problems.
- Currently focusing on **Deep Learning, neural networks and advanced ML**.
- I work with **Python, NumPy, Pandas, Scikit-learn and data-driven workflows**.
- I practice **Data Structures & Algorithms** and maintain my LeetCode journey.
- Interested in **anomaly detection, recommendation systems, AI research and intelligent applications**.
- My goal is to turn ML models into **reliable end-to-end AI systems**.

<details>
<summary><b>Current Learning</b></summary>

`Machine Learning` → `Deep Learning` → `Generative AI` → `AI Engineering`

</details>

---

## Tech Stack

### Languages

`Python` `C` `C++` `SQL`

### Machine Learning / Deep Learning

`Scikit-learn` `TensorFlow` `Keras`

### Data / Analytics

`NumPy` `Pandas` `Matplotlib` `Power BI`

### Backend / Tools

`FastAPI` `MySQL` `Firebase` `Git` `GitHub` `Jupyter` `VS Code`

---

## Dynamic Project Universe

> Automatically generated from my public GitHub repositories. New public repositories are picked up automatically by GitHub Actions.

<!--START_PROJECTS-->

| 🚀 Project | 📝 Description | ⭐ Stars | 🛠️ Technologies |
|---|---|---:|---|
| [🚀 Leetcode](https://github.com/ChiragGandhi2006/LeetCode) | A collection of LeetCode questions to ace the coding interview! - Created using [LeetHub v2](https://github.com/arunbhardwaj/LeetHub-2.0) | ⭐ 0 | C++ |
| [🚀 Github Final Project](https://github.com/ChiragGandhi2006/github-final-project) | No description yet. | ⭐ 0 | Shell |
| [🚀 Ai Based Cloud Removal And Spectral Reconstruction Of Satellite Imagery](https://github.com/ChiragGandhi2006/AI-Based-Cloud-Removal-and-Spectral-Reconstruction-of-Satellite-Imagery) | No description yet. | ⭐ 0 | Python |
| [🚀 Finplot Ai](https://github.com/ChiragGandhi2006/FinPlot-AI) | No description yet. | ⭐ 1 | JavaScript |
| [🚀 Movie Recommendation System](https://github.com/ChiragGandhi2006/Movie-Recommendation-System) | No description yet. | ⭐ 1 | Jupyter Notebook |
| [🚀 Autonomous Ai Project Partner](https://github.com/ChiragGandhi2006/Autonomous-AI-Project-Partner) | Autonomous AI project partner for Hackathon and Dev teams | ⭐ 1 | Python |
| [🚀 Airline Reservation System Dbms](https://github.com/ChiragGandhi2006/airline-reservation-system-dbms) | No description yet. | ⭐ 1 | Python |

<!--END_PROJECTS-->

---

## Featured Projects

### [Movie Recommendation System](https://github.com/ChiragGandhi2006/Movie-Recommendation-System)

A practical recommendation-engineering project combining machine learning, recommendation techniques and an API/backend layer. One of my strongest projects for AI/ML and placement discussions.

### [Autonomous AI Project Partner](https://github.com/ChiragGandhi2006/Autonomous-AI-Project-Partner)

An AI-focused development project designed around autonomous project assistance, demonstrating my interest in intelligent applications beyond traditional notebooks.

### [FinPlot-AI](https://github.com/ChiragGandhi2006/FinPlot-AI)

An AI-oriented financial analysis project demonstrating the application of intelligent systems to a real-world domain.

### [AI-Based Cloud Removal & Spectral Reconstruction](https://github.com/ChiragGandhi2006/AI-Based-Cloud-Removal-and-Spectral-Reconstruction-of-Satellite-Imagery)

A research-oriented project applying AI to satellite imagery, giving my profile a strong computer-vision and remote-sensing direction.

### [Airline Reservation System](https://github.com/ChiragGandhi2006/airline-reservation-system-dbms)

A full application demonstrating Python, GUI development, MySQL, CRUD operations, booking workflows and QR-based boarding passes.

---

## GitHub Analytics

- GitHub profile: [ChiragGandhi2006](https://github.com/ChiragGandhi2006)
- Repositories: dynamically maintained by GitHub Actions
- Project discovery: automatically updated from public repositories

---

## LeetCode Journey

My LeetCode profile: [CKhjBWYKSg](https://leetcode.com/u/CKhjBWYKSg/)

I use LeetCode to strengthen **DSA, problem solving and interview preparation**.

---

## Learning Roadmap

`Python` → `Machine Learning` → `Deep Learning` → `Generative AI` → `AI Engineering`

| Area | Focus |
|---|---|
| Machine Learning | Advanced algorithms and optimization |
| Deep Learning | ANN, CNN and neural networks |
| Generative AI | LLM fundamentals and applications |
| Data Science | Analysis and feature engineering |
| DSA | Problem solving and interviews |
| AI Engineering | End-to-end intelligent systems |

---

## Achievements & Research

- **1st Prize — INC IoT Project Execution Competition, PICT Pune (2025)**
- **Research:** *Credit Card Fraud Detection Using Hybrid XGBoost and Autoencoder Models* — GR Journal, 2026
- **CGPA: 9.27/10.0** in B.Tech AI & Data Science
- **NVIDIA:** Applications of AI for Anomaly Detection
- **C-DAC:** Artificial Intelligence

---

## Connect With Me

[LinkedIn](https://www.linkedin.com/in/chirag-gandhi-672a67372/) • [GitHub](https://github.com/ChiragGandhi2006) • [LeetCode](https://leetcode.com/u/CKhjBWYKSg/) • [Email](mailto:gandhichirag2006@gmail.com) • [Resume](https://github.com/ChiragGandhi2006/ChiragGandhi2006/blob/main/RESUME.md)

---

## Developer Philosophy

> **Learn → Build → Break → Debug → Improve → Repeat.**

**Don't just learn AI. Build with it.**

---

<div align="center">

### Thanks for visiting my profile!

</div>
